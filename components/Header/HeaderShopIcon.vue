...
import { apiShopCartItemListRetrieve } from "@/api";
import type { CardItems } from "@/types";

const authStore = useAuthStore();

const { data: cardItems } = useLazyAsyncData(
  "card-items",
  async () => (await apiShopCartItemListRetrieve()) as unknown as CardItems,
  {
    ...
    immediate: Boolean(authStore.isAuthenticated),
  }
);

const basketItemsCount = computed(() => cardItems.value?.length ?? 0);
</script>
<template>
  <div class="relative">
    <svg
